...
//                .diskCacheFileNameGenerator(new Md5FileNameGenerator())
    ...
    private void initImgFromUrl(String[] imageUrl) {
      ...
      //   dotView.setImageResource(R.drawable.logo);
            dotLayout.addView(dotView, params);
            imagesDots.add(dotView);
        }
//        imagesDots.get(0).setImageResource(R.drawable.logo);
    ...
    class LoopPicturesListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            for (int i = 0; i < imagesDots.size(); i++) {
                if (i == position - 1) {
                    imagesDots.get(i).setImageResource(R.drawable.yuan_red);
                    if (hasTitle) {
//                        titleTV.setText(imageTitle[i]);//设置文字，没有就会报空
                    }
                } else {
                    imagesDots.get(i).setImageResource(R.drawable.yuan_write);
                }
            }
